...
async function _get(baseUrl: string, apiKey: string, secretKey: string, uriPath: string, payload: string, params: any = {}) {
...
export async function getAccountBalance(baseUrl: string, apiKey: string, secretKey: string, page: number = 1, perPage: number = 10, usePagination: boolean = false, asset: string  = "") {
    const payload = "";
...
export async function getOpenOrders(baseUrl: string, apiKey: string, secretKey: string, instrument: Instruments = "btc_usdt_spot", fromTime: number = 0, toTime: number = currentDateNs) {
...
export async function getClosedOrders(baseUrl: string, apiKey: string, secretKey: string, instrument: Instruments = "btc_usdt_spot", fromTime: number = 0, toTime: number = currentDateNs) {
    const payload = JSON.stringify({
        instrument,
        from_time: fromTime,
        to_time: toTime
...
export async function getTradeHistory(baseUrl: string, apiKey: string, secretKey: string, instrument: Instruments = "btc_usdt_spot", count: number = 1, fromTime: number = 0, toTime: number = currentDateNs) {
    const payload = JSON.stringify({
        instrument,
        count,
        from_time: fromTime,
...
export async function cancellingOpenOrderPerInstrument(baseUrl: string, apiKey: string, secretKey: string, instrument: Instruments = "btc_usdt_spot") {
    const payload = JSON.stringify({
        instrument
    });
...
export async function cancellingOpenOrderPerOrderIds(baseUrl: string, apiKey: string, secretKey: string, orderIds: string[], qtys: number[]) {
    const payload = JSON.stringify({
        order_ids: orderIds,
        qtys
    });
...
export async function openingNewOrder(baseUrl: string, apiKey: string, secretKey: string, instrument: Instruments, side: number, orderType: number, orderQuantity: string, price?: number) {
    if (orderType == 1) {
        price = 0;
    }
